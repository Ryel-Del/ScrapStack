...
import { 
  Search, MapPin, Tag, Box, Cpu, AlertCircle, 
  Layers, ChevronRight, ChevronLeft, CheckCircle2, 
  Loader2, CreditCard 
} from 'lucide-react';
...
export default function Marketplace() {
  ...
  // --- PAYMENT HANDLER ---
  const handlePayment = async (itemId: string) => {
    setProcessingId(itemId);
    // Simulate payment delay
    await new Promise((resolve) => setTimeout(resolve, 2000));
    setProcessingId(null);
    setPaymentSuccess(itemId);
    // Reset success UI after 3 seconds
                      ...
                      <div className="mt-auto flex justify-between items-end gap-3">
                        ...
                        <div className="flex gap-2">
                          <Link href={`/marketplace/${item.id}`}>
                            <button className="bg-slate-100 text-slate-900 px-4 py-4 rounded-2xl text-[10px] font-black hover:bg-slate-200 transition-all">
                              MANIFEST
                            </button>
                          </Link>
                          ...
                          <button 
                            ...
                            disabled={processingId === item.id || paymentSuccess === item.id}
                            className={`flex items-center gap-2 px-6 py-4 rounded-2xl text-[10px] font-black transition-all shadow-lg active:scale-95 min-w-[120px] justify-center ${
                              paymentSuccess === item.id 
